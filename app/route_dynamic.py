...
def Supported_Countries():
    ...
    form_dynamic_data[user_id] = {"age_over_18": True}
    form_dynamic_data[user_id].update(
        {"expires": datetime.now() + timedelta(minutes=cfgserv.form_expiry)}
    )

    return redirect(
        url_for("dynamic.redirect_wallet", user_id="AV." + user_id, jws_token=jws_token)
    )
...
def dynamic_R2():
    """Route acessed by OpenID to get PID attributes from country AV
...
def credentialCreation(credential_request, data, country):
    ...
    document_mappings = cfgserv.dynamic_issuing
